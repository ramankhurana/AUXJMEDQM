...
void AUXJMEDQMSummary(TString rootfileList, TString postname)
{
  ...
  for (int ii=0; ii<(int)lines.size(); ii++){
    ...
    if (TString(str_component_pdf[2].c_str()) == "MET") {
      pdfname = TString(str_component_pdf[2].c_str()) + "_" + TString(str_component_pdf[3].c_str()) + "_" + TString(str_component_pdf[4].c_str()) + "_" + TString(str_component_pdf[5].c_str()) +".pdf";
    }
  ...
  TH2F *h1 = new TH2F("h1","Mean Values",(nFiles+1)*16,0,(nFiles+1),24000,0.5*smallmean,2*largemean);
  ...
  c1->SaveAs("Plots/mean_"+pdfname.ReplaceAll(".pdf",".png"));
  ...
  TH2F *h2 = new TH2F("h2","RMS",(nFiles+1)*16,0,(nFiles+1),6000,0.5*smallRMS, 2*largeRMS);
  for (Int_t i=1;i<(nFiles+1);i++) {
    cout << RMS[i] << endl;
    h2->Fill(i, RMS[i-1]);
  }
  ...
  //    c2->SaveAs("Plots/rms_"+pdfname.ReplaceAll(".pdf",".png"));
